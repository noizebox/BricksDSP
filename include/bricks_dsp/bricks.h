#ifndef BRICKS_DSP_BRICKS_H
#define BRICKS_DSP_BRICKS_H

namespace bricks_dsp {
constexpr int VERSION_MAJOR = BRICKS_DSP_VERSION_MAJOR;
constexpr int VERSION_MINOR = BRICKS_DSP_VERSION_MINOR;
}

#include "dsp_brick.h"
#include "analyzer_bricks.h"
#include "envelope_bricks.h"
#include "filter_bricks.h"
#include "modulator_bricks.h"
#include "oscillator_bricks.h"
#include "utility_bricks.h"

#endif //BRICKS_DSP_BRICKS_H
